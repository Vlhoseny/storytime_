...
<body>
      ...
      <div id="main-contents">
        <div>
          ...
          <form action="" method="post" class="add-admin-form" style="margin-top: 15px;">
            ...
            <select name="Book Type" class="input" style="height: 50px;">
              <option value="Novel">Novel</option>
              <option value="Book">Book</option>
              <option value="Manga">Manga</option>
            </select>

            <button class="add-admin" name="action" type="submit" value="submit"> Submit! </button>
          </form>
        </div>
      </div>
  ...
  if (@$_POST['action']) {
    if ($_POST['action'] == 'submit') {
      ...
      $sql = "UPDATE books SET Book_title='$Book_Title', Book_Description='$Book_Description', 
      Book_price = '$Book_Price', book_type = '$Book_Type', Book_genre = '$Book_Genres' WHERE Book_id=$id";
